...
# Wechat DevTools MCP Skill (v0.7.0)
...
| 改了代码 | `compile` → `start` → `page_data` 验证 | 重新 open → compile |
...
- **注意**：v0.7.0 起支持声明语句（const/let/var）；v0.6.0 仅支持表达式，多行需用 IIFE
...
wechat_automator(action='start')                    # compile 后重连 automator
...
```
wechat_build(action='compile')                      # 仅编译
wechat_automator(action='start')                    # compile 后重连
wechat_automator(action='page_data')                # 验证连接可用
```
...
| tabBar / 强制重置 | `evaluate(expression="wx.reLaunch({url:'<path>'}); 'ok'")` |
...
wechat_build(action='compile')                      # 重编译
wechat_automator(action='start')                    # compile 后重连
wechat_automator(action='page_data')                # 验证
```
...
| `Connection closed` 截图/操作失败 | automator WS 连接断开 | 先快速恢复（仅 `start` → `page_data`）；失败再完整恢复（见 Recovery Tiers） |
...
| switchTab ok 但未切换 | switchTab 异步未完成 | 改用 wx.reLaunch |
...
- ❌ compile 后不重新 start automator（编译重置运行时，必须重连）
