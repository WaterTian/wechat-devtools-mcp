...
| 版本 | 说明 |
|------|------|
| **0.9.18** | **Windows 2.x 真机闭环 + 稳定性**（2026-09-04）：在 Windows 11 真机（Stable 2.02.2608060）完成 4 轮冒烟 19/19 与全量 pytest。**状态文件目录**：2.x Windows 在 `%LOCALAPPDATA%\微信开发者工具\User Data\<hash>\Default\`，`ide_state` 对每个候选根同时探测有 / 无 `User Data` 两层。**open**：Windows Electron 不再把 stale 的 `.ide` 端口当就绪判据，改为「CDP 已监听 + `/json/list` 出现 IDE 外壳 target」，端口不通也放行、以 `cli open` 实际结果为准；新增返回 `miniprogram_targets_ready`，8 秒内看不到小程序 target 时 message 带 ⚠（单实例锁竞态探针），不判失败。**启动健康检查噪音**：过滤 2.x 扩展注入的 `inspectee MPPage` 与登录过期的 `access_token expired`。**compile 噪音**：`(Use \`<可执行名> --trace-deprecation` 提示按内容匹配，不再写死可执行名（Windows 是 `微信开发者工具`）。**quit / kill**：Windows 改为 `tasklist /FO CSV` 轮询进程消失（按行首引号判定，不依赖 GBK 码页），`quit` 返回 `exited`。**测试与文档**：新增跨平台真机冒烟 `tests/manual/smoke_ide.py`（JSON 报告 + 检查点）；两个 Windows 单测解耦真实安装目录；新增 `docs/windows-internals.md`（安装包静态解包 + 真机核对）。测试 250 → 266 |
