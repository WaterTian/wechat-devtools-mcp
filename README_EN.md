# WeChat DevTools MCP Server (v0.9.16)
...
| `wechat_inspector` | Runtime log collection | `console` `cdp` `network` |
...
| Version | Description |
|---------|-------------|
| **0.9.17** | **Network.enable observation**: `wechat_inspector(action='network')` captures real `wx.request` traffic from the appservice target, with URL regex filtering, optional POST data and response status. Sensitive query/body fields are redacted and unavailable CDP or unsupported Network domains return explicit error codes. |
