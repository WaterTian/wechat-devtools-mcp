...
| `wechat_inspector` | 运行时日志采集 | `console` `cdp` `network` |
...
| 版本 | 说明 |
|------|------|
| **0.9.17** | **Network.enable 网络观测**：`wechat_inspector(action='network')` 可采集 appservice 逻辑层发起的真实 `wx.request`，支持 URL 正则筛选、可选 POST 请求体与响应状态；敏感 query/请求体字段自动脱敏，CDP 不可用或 Network 域不支持时返回明确错误码。 |
